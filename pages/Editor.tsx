  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-48">
        ...
        <p className="text-gray-400 font-bold uppercase tracking-widest text-xs">{t.editor.preparing}</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative pb-24 md:pb-0">
      ...
      <motion.div 
        ...
        className="hidden md:flex items-center justify-between mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100"
      >
        <div className="flex items-center space-x-4">
          <button 
            onClick={() => navigate(-1)} 
            className="group inline-flex items-center text-sm font-bold text-gray-400 hover:text-indigo-600 transition-colors"
          >
            <ArrowLeft size={18} className="mr-2 group-hover:-translate-x-1 transition-transform" /> 
            {t.editor.cancel}
          </button>
          <div className="h-5 w-[1px] bg-gray-200"></div>
          <span className="px-3 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-full flex items-center">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center px-6 py-2 border-2 border-indigo-300 bg-indigo-50 text-indigo-600 rounded-xl font-bold hover:bg-indigo-100 disabled:opacity-50 transition-all text-xs"
          >
            {saving ? (
      ...
      <motion.div 
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="md:hidden flex items-center justify-between mb-4 bg-white p-3 rounded-xl shadow-sm border border-gray-100"
      >
        <button 
          onClick={() => navigate(-1)} 
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div 
          ...
          className="bg-white rounded-2xl md:rounded-[3rem] shadow-sm border border-gray-100 p-4 md:p-10"
        >
          {!previewMode ? (
            <div className="space-y-4 md:space-y-6">
              <input
                ...
                className="w-full text-2xl md:text-4xl lg:text-5xl font-black text-gray-900 border-none focus:ring-0 bg-transparent placeholder-gray-300 outline-none"
              />
              ...
              <textarea
                ...
                className="w-full min-h-[60vh] md:min-h-[70vh] p-0 text-base md:text-lg leading-relaxed text-gray-700 border-none focus:ring-0 bg-transparent resize-none placeholder-gray-300 outline-none"
              ...
              >
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <select 
                    ...
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-medium"
                  >
                ...
                {/* 统计信息 */}
                <div className="bg-gradient-to-br from-indigo-50 to-blue-50 p-6 rounded-2xl border border-indigo-100 md:col-span-3">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-2xl font-black text-indigo-600">{wordCount}</div>
                      <div className="text-xs text-gray-600 mt-1">{t.editor.wordCount || '字数'}</div>
                    </div>
                    <div>
                      <div className="text-2xl font-black text-indigo-600">{estimatedReadTime}</div>
                      <div className="text-xs text-gray-600 mt-1">{t.editor.readTime || '分钟'}</div>
                    </div>
                    <div>
                      <div className="text-lg font-black text-indigo-600">{new Date().toLocaleDateString('zh-CN', { month: 'short', day: 'numeric' })}</div>
                      <div className="text-xs text-gray-600 mt-1">{t.editor.date || '日期'}</div>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      >
        <div className="flex items-center justify-between p-3">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex items-center px-6 py-3 border-2 border-indigo-300 bg-indigo-50 text-indigo-600 rounded-xl font-bold hover:bg-indigo-100 disabled:opacity-50 transition-all"
          >
            {saving ? (
        ...
        <AnimatePresence>
          {showSettings && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              className="border-t border-gray-200 p-4 space-y-4 bg-gray-50"
            >
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-2">
                  {t.editor.category}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder={t.editor.excerptHint}
                  className="w-full px-3 py-2 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none text-sm resize-none"
                />
              </div>
            </motion.div>
          )}
